      ...
      >
        ...
        >
          ...
          <el-form-item label="确认密码" prop="checkPassword">
            <el-input
              type="password"
              v-model="editContent.checkPassword"
              autocomplete="off"
            ></el-input>
          </el-form-item>
        </el-form>
    ...
    <div class="tag-container">
        <div class="tag-header-container">
            <el-dialog
                ...
                @close="editUserVisible = false"
            >
                <el-form :model="editContent" :rules="rules" ref="ruleForm" label-position="right" label-width="80px">
                    <el-form-item label="用户名" prop="name">
                        <el-input v-model="editContent.name"></el-input>
                    </el-form-item>
                    <el-form-item label="密码">
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button @click="$emit('close')">取 消</el-button>
                    <el-button :loading="editLoading" type="primary"
                               @click="confirmEdit(editContent)">确定</el-button>
                  </span>
            </el-dialog>
        </div>
        <el-table
            ...
            confirmEdit() {
                this.$refs.ruleForm.validate(async (valid) => {
                    if (valid) {
                        ...
                        this.editLoading = true
                        try {
                            const result = await update(this.editId, this.editContent.name,
                                this.editContent.password)
                            if (result.data.code) {
                                this.$message.error('编辑失败')
                            } else {
